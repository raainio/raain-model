...
describe('Radar', () => {
    ...
    it('should create ones', () => {
        ...
        expect(JSON.stringify(radarNode.toJSON())).eq(
            '{"id":"RadarNode looks OK.","links":[],"name":"name","latitude":1,"longitude":1,"team":"tid1"}'
        );
    });
    ...
    it('should transform Polar', () => {
        ...
        const radarPolarMeasureValue1 = new RadarPolarMeasureValue({
            polarMeasureValue,
            angle: 1,
            axis: 0,
        });
        ...
        const radarPolarMeasureValue2 = new RadarPolarMeasureValue({
            polarMeasureValue: radarPolarMeasureValue1,
            angle: 1,
            axis: 0,
        });
        expect(radarPolarMeasureValue2.getAzimuthsCount()).equal(720);
        expect(radarPolarMeasureValue2.getPolarEdgesCount()).equal(250);
        expect(JSON.stringify(radarPolarMeasureValue2.toJSONWithPolarStringified())).eq(
            JSON.stringify(radarPolarMeasureValue1.toJSONWithPolarStringified())
        );
        expect(JSON.stringify(radarPolarMeasureValue2.toJSONWithPolarStringified())).eq(
            '{"polarMeasureValue":"{\\"measureValuePolarContainers\\":\\"[{\\\\\\"azimuth\\\\\\":0,\\\\\\"distance\\\\\\":1,\\\\\\"polarEdges\\\\\\":[33,45.5],\\\\\\"edgeOffset\\\\\\":0}]\\",\\"azimuthsCount\\":720,\\"polarEdgesCount\\":250}","angle":1,"axis":0}'
        );

        const radarPolarMeasureValue3 = new RadarPolarMeasureValue(
            radarPolarMeasureValue1.toJSONWithPolarStringified()
        );
        expect(radarPolarMeasureValue3.getAzimuthsCount()).equal(720);
        expect(radarPolarMeasureValue3.getPolarEdgesCount()).equal(250);
        expect(JSON.stringify(radarPolarMeasureValue3.toJSONWithPolarStringified())).eq(
            JSON.stringify(radarPolarMeasureValue1.toJSONWithPolarStringified())
        );
        expect(JSON.stringify(radarPolarMeasureValue3.toJSONWithPolarStringified())).eq(
            '{"polarMeasureValue":"{\\"measureValuePolarContainers\\":\\"[{\\\\\\"azimuth\\\\\\":0,\\\\\\"distance\\\\\\":1,\\\\\\"polarEdges\\\\\\":[33,45.5],\\\\\\"edgeOffset\\\\\\":0}]\\",\\"azimuthsCount\\":720,\\"polarEdgesCount\\":250}","angle":1,"axis":0}'
        ...
        const radarMeasure = new RadarMeasure({
            id: 'measureId',
            values: [polarMeasureValue],
            date: new Date(100000),
        });
        expect(JSON.stringify(radarMeasure.toJSON())).eq(
            '{"id":"measureId","links":[],"date":"1970-01-01T00:01:40.000Z","validity":-1,"values":[{"measureValuePolarContainers":[{"azimuth":0,"distance":1,"polarEdges":[33,45.5],"edgeOffset":0}],"azimuthsCount":720,"polarEdgesCount":250}]}'
        );

        const polarMeasureValue1 = radarMeasure.values[0] as PolarMeasureValue;
        const radarPolarMeasureValue4 = new RadarPolarMeasureValue({
            polarMeasureValue: polarMeasureValue1,
            angle: 1,
            axis: 0,
        });
        expect(JSON.stringify(radarPolarMeasureValue4.toJSON())).eq(
            '{"polarMeasureValue":{"measureValuePolarContainers":[{"azimuth":0,"distance":1,"polarEdges":[33,45.5],"edgeOffset":0}],"azimuthsCount":720,"polarEdgesCount":250},"angle":1,"axis":0}'
        );
    ...
    it('should use multi-dimension image', async () => {
        const testOneFake = (move: number) => {
            const radarPolarMeasureValues =
                RadarPolarMeasureValue.BuildFakeRadarPolarMeasureValues(move);
            ...
            optionalTable(
                polars
                    .filter((p, pi) => pi % 10 === 0)
                    .map((m) => m.polarEdges.filter((n, ni) => ni % 10 === 0))
            );
        };
